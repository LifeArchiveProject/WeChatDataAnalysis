...
class TestVoiceTranscriptionContract(unittest.TestCase):
    ...
    def test_desktop_backend_build_includes_optional_runtime(self):
        ...
        self.assertIn('"wce_integrity.pyd"', build)
        self.assertNotIn("windowsNativeCandidates", build)
        self.assertIn('"Scripts/pyinstaller.exe"', build)
        self.assertIn("spawnSync(pyInstallerExecutable", build)
        self.assertIn('"Scripts/python.exe"', build)
        self.assertIn("const integrityPreflight = spawnSync(\n  venvPythonExecutable", build)
        self.assertNotIn('spawnSync(\n  "uv"', build)
        self.assertIn('"src/wechat_decrypt_tool/native/*.pyd"', pyproject)
